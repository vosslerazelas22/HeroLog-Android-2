...
| `HeroLogModal` (componente de modal reutilizável) | **Em correção (spec-002)** — passos 1–2b fechados 16/09: Opção A adotada em device (raio 0.25 dp noise 0; Voltar corrigido; API 24 fallback chapado); sizing validado nos 2 viewports (painel pelo conteúdo; glow `matchParentSize` era o 2º esticador); backdrop blur ok no fundo e header. Resta a tabela visual §4 (passo 3: Cinzel, sombra, paddings, glow 1%, fechar, easing) | Fiel — props, `ModalCountRegistry`, `BackHandler`-dentro-do-`Dialog` com guard `isOpen` (BH-1 fechado). Device: Voltar fecha (desbloqueado) / não fecha (bloqueado), incl. produção | **Parcial — sizing + backdrop validados em device (2 viewports); §4 pendente** — baseline Roborazzi registra o bug antigo, não é prova | Bloco 3 + 3.1. Port de `Modal.tsx`. `incrementModalCount`/`decrementModalCount` resolvidos (Bloco 6.2, `ModalCountRegistry`). D1–D11 mapeados na spec-002. Backlog mesma causa raiz: `LevelUpOverlay.kt` (sprint própria). Ver `DEV_LOG_ANDROID.md` 16/09 |
