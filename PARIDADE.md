...
| Scaffold background (`containerColor`) | **Fechado (09/09)** — `QuestPanel` (#0B0915) definido em `Color.kt`, Scaffold usa `containerColor = QuestPanel`, 12 telas tiveram `.background(Stone950)` removido | N/A (chrome de UI) | **Não validada** — alinhamento com React `bg-quest-panel` (#0B0915) confirmado por código; inspeção visual em device pendente | React: `bg-quest-panel` (#0B0915) em todas as abas (`App.tsx:2332-3518`, `HabitsTab.tsx:103`, `DailiesTab.tsx:128`, `TodosTab.tsx:123`). `QuestPanel` consolidada em `Color.kt` (antes duplicada em `AppHeader.kt:93` e `GeneralSettingsModal.kt:72`). 12 telas: Habits, Dailies, Todos, Quests, History, Shop, TitleSelector, Heatmap, Stats, Achievements, Guide, Logs. LogsScreen também teve cor `#1A1614` + `RoundedCornerShape` + `border` removidos da raiz. **Pendência**: wrappers de Character/Inventory em MainActivity (Column com `Stone900@50%` + border) ficam para bloco próprio futuro — repercussões no header interno "FICHA DO HERÓI"/"EQUIPAMENTOS". |

## 3. Módulo Foco

| Item | Status | Fidelidade Lógica | Fidelidade Visual | Notas |
|---|---|---|---|---|
| Timer Pomodoro (controles de sessão) | **Completo** — wiring real na `MainActivity` (Blocos 35/36), UI fiel do timer de descanso (`BreakPrepScreen` + `FocusOrb` variante esmeralda, Bloco 41), reset de `isDungeonMode`/`dungeonSessionsProgress` ao completar a 4ª masmorra e ao abandonar sessão (Bloco 42), **break prep/running shell persistente + fullscreen break (Bloco E, 14/09)** — todos validados visualmente por Bruno de ponta a ponta | Validada — `FocusApplyLogic.apply` chamado de fato em `confirmFocusSession`, confirmado com valores reais em screenshots humanas. `startBreakTimer`/`confirmFocusSession` decidindo curta/longa duração de descanso fiel a `handleConfirmClaimRewards` real (Bloco 39). `abandonSession()` nova (Bloco 42), isolada de `cancelSession()` para não interferir na chamada de segurança interna de `startBreakTimer()` — replica os dois pontos de reset da fonte real (`handleConfirmClaimRewards` linha ~1483-1499 E `handleAbandonSession` linha ~1241-1248). 83/83 testes sem regressão (Bloco E). **Bloco E (14/09)**: D24/D25 corrigidos — break-prep/running agora preservam banner "CÂMARA DE FOCO" + `SkillInlineCarousel` + `QuickActionsBar`; fullscreen break espelha React (`App.tsx:2055`); `BreakTimerState` agora captura `wasLastSessionWildernessMode`/`lastSessionDungeonSessions`; `isRunning=false` em todos os estados de break (match React `enterBreak`); break-orb/`RaidModeInfoBox`/`QuickActionsBar` usam `breakTimerState.wasLastSession*` em vez de preview toggles | **Confirmada por inspeção visual humana real** — fluxo completo incluindo timer de descanso (com e sem autostart) e os dois cenários de reset do segmented control (conclusão da 4ª sessão e abandono no meio), 4 prints reais (18/08). **Visual do Bloco E (D24/D25): PENDENTE** — aguardando inspeção em device/emulador | React: `App.tsx` + `useFocusSession.ts` + `useBreakTimer.ts` (fonte real obtida 16/08). SFX (`audio.ts`) e som ambiente (`useAmbientSound.ts`) fora de escopo por decisão explícita — fontes reais já obtidas e registradas pra quando retomado. **Divergência de UX registrada, fora de escopo por ora**: React exige clique duplo em 5s para confirmar abandono de sessão (`isConfirmingAbandon`) + log de sistema "FRACASSO NA MASMORRA"; Android sai direto no primeiro clique, sem confirmação nem log (não há infraestrutura de log de sistema real ainda). **Correção de texto (07/09)**: botões Pausar/Abandonar/Reto... **Correção visual (11/09, Bloco TooltipButtons)**: Iniciar Missão fontSize 13→14sp, padding 14→12dp, shape 6→4dp, +Cinzel; Pausar/Retomar +Cinzel, letterSpacing 0.8→1.5sp (tracking-widest); botão "?" 20→18dp, tooltip movido para Popup overlay (paridade React position:absolute). 491/491 testesmar/Confirmar corrigidos de ALL CAPS para sentence case (`"RETOMAR MISSÃO"` → `"Retomar Missão"`, `"CONFIRMAR?"` → `"Confirmar?"`, etc.), alinhando com `App.tsx:2775,2785` e `FocusModeScreen.tsx:215` | **Bug #5 corrigido (10/09)**: botão "CONTINUAR"/"RECEBER RECOMPENSAS" ficava fora da viewport em telas pequenas (375×667, 390×844). `CompletionShell` reescrito: Box→Column, `weight(1f)` no conteúdo, `LocalBottomBarInset.current` no bottom (padrão do projeto, 12 telas), `imePadding()` para teclado. `adjustResize` no Manifest. 5 testes Roborazzi novos (375×667 e 390×844). **Visual: PENDENTE** — aguardando inspeção em device |
...
| FocusCompletionFlow (SessionSummary — D173/D174) | **Validado (código + testes, 11/09)** — título 20sp Cinzel ExtraBold + letter-spacing 3sp, rank label `★ CLASSIFICAÇÃO X ★` 14sp Cinzel com drop-shadow Champagne400/35%, descrição 10sp JetBrainsMono uppercase, divider 85% Amber500/15%. Stretch D175–D181, D4–D8 fora de escopo | Validada — lógica idêntica ao React (getRank/getRankDescription pure functions, mesmos thresholds) | **Parcial** — D173/D174 implementados + drop-shadow; validação visual em device pendente | React: `FocusCompletionFlow.tsx` linhas 180–192. Bug #5 (botão fora da viewport) já corrigido no Bloco anterior (CompletionShell reescrito). **Simplificações registradas**: drop-shadow aplicado no Column wrapper (glow envolve label + descrição), não no Text do rank label como no React (diferença visual mínima) |
...
| Guild Quests + Proclamações diárias (20 quests, rotação LCG determinística) | **Removido (17/09, spec-001)** — GUILD_QUESTS removido do QuestCatalog, guildQuestsProcessed removido do ViewModel, subaba "Marcos da Jornada" removida do QuestsScreen e QuestFab. IDs legados claimed_guild_* permanecem inertes em saves existentes. Decisão consciente: Android-first, divergência registrada | N/A | N/A | Remoção validada em emulador 390×844: QuestsScreen mostra apenas contratos diários (CAPELA DE HÁBITOS, TAREFAS DIÁRIAS, MISSÕES AVULSAS, CONTRATOS, CRÔNICAS DIÁRIAS). PR #12 |
...
| Conquistas (42 itens + detecção central + anúncios) | **Implementado (17/09, spec-001, PR #12)** — AchievementCatalog expandido de 8→42 itens, AchievementDetection pura/idempotente/sem retroativos, etapa "Conquista desbloqueada" no FocusCompletionFlow (entre Loot e Notas), AchievementAnnouncementOverlay (fila global efêmera para unlocks fora do foco), pré-detecção em onFocusSessionCompleted | Validada — 50 testes catálogo + 11 detecção + 6 integração (557/557 total) | **Validada** — emulador 390×844: FocusCompletionFlow com etapa de conquista, QuestsScreen sem guild, catálogo 42 itens visível | Decisão consciente: Android-first (catálogo 42 vs 8 do React, sem recompensa econômica). Detecção só em fluxos explícitos (foco/hábito/título); importação não dispara (FR-007). BackHandler não usado no overlay (dispatcher de janela — pendência device §9.6). Commit 84cb63e corrige bug: pré-detecção em onFocusSessionCompleted (antes só em confirmFocusSession) |
