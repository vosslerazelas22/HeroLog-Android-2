...
| `normalizeGameState` / migração de save antigo | **Fechado (27/08)** — lógica (`SaveMigrationLogic.kt`) + UI de import (`RestoreSaveDialog.kt`) integradas, commit `24b3a26`. Build real confirmado via XML cru: `SaveMigrationLogicTest` 12/12 e `HeroLogViewModelTest` 52/52 (suíte grande, tocada pela nova `importSaveFromPastedText`) — 0 falhas/erros/skips nas duas, nenhuma regressão | Validada — `SaveMigrationLogic.kt` (`normalizeGameState`). Auditado contra `useGameState.ts`/`App.tsx` reais (commit `52822c2`): spread raso `{...INITIAL_STATE, ...parsed}` reusando `createInitialCharacterState()` já existente, + migrações específicas (pomodoroSettings com fallback pro campo morto `longBreakMinutes`, `skills[].id/emoji/prestige` com defaults determinísticos `sk-N`/💻🧪📚🎯 — não UUID aleatório, fiel à fonte —, `dailies[].value/createdAt`, `todos[].createdAt`, guarda de `equippedEquipment`) | Não validada formalmente (sem print lado a lado) | React: `useGameState.ts` (`normalizeGameState`/`importGameState`) + `App.tsx` (`handleImportSaveFromText`, modal "Restaurar Código Rúnico"). **Recorte deliberado**: só a via "código de salvamento colado" foi portada, não o modal "Ajustes Gerais" inteiro (edição de nome do herói/dispositivo, export/import por arquivo `.json`, copiar pra área de transferência, purgar campanha, logout) — nenhum desses tem equivalente de conta/Supabase no Android (fora de escopo, ver linha "Sync remoto" acima). Entry point novo: ícone de engrenagem na `TopAppBar` (`MainActivity.kt`), único ponto de configurações que existe no app até agora. **🔴 28/08: esse `TopAppBar` foi substituído pelo `AppHeader` — ver linha `AppHeader` na seção 2 pro risco de regressão (entry point pro `RestoreSaveDialog` pode ter sumido)**. `orbConcept` do `INITIAL_STATE` real (sem equivalente em `CharacterState.kt`) é ignorado na decodificação (`ignoreUnknownKeys = true`) — divergência de modelo pré-existente, não introduzida aqui. Nota: `HeroLogModal(...)` em `RestoreSaveDialog.kt` foi chamado sem o parâmetro `modifier` (não existe na assinatura real do componente) — ajuste feito pelo AI Studio na hora de compilar, não pedido por mim |
...
| Quick Actions Bar (Modo/Som/Ajustes/Tela Cheia) | **Validado (29/08)** — desacoplamento `isRunning`/`isFocusMode` implementado (sessão roda inline na aba Foco, tela cheia é ação explícita), 4 botões wireados. **Bug real corrigido**: `onExit` do `FocusModeScreen` abandonava a sessão ao sair da tela cheia — deveria só fechar o overlay; sessão segue rodando. Confirmado por Bruno no device | Fiel — porte do "Quick Actions Row" do `App.tsx`. Ícones migrados de Material Icons aproximados para `com.composables:icons-lucide-android:2.2.1` (317★, `isTransitive = false`) após avaliação de 3 alternativas (`thelacspace/lucide-compose` 1★ descartada, `dead8309/lucide-kotlin` descartada por ser wrapper Kotlin/JS de `lucide-react`, não gerar recurso Android nenhum). Todos os 12 ícones conferidos contra lista explícita aprovada por Bruno: `Sparkles`, `Shield`, `ShieldAlert`, `Volume2`, `VolumeX`, `Settings`, `Maximize`, `Pause`, `Play`, `X`, `ToggleLeft`, `ToggleRight` | Confirmado manualmente por Bruno no device (29/08) — header, quick actions idle e sessão rodando, modal de som, Pausar/Abandonar inline | React: `App.tsx` (`herolog-brainstorm-quickactions-bar.md`). **Nenhum teste de screenshot dedicado existe** pra `AppHeader`/`QuickActionsBar`/`AmbientSoundModal` — cobertura automatizada da migração de ícone é zero por design de teste atual, validação é 100% manual (aceito, ver `DEV_LOG_ANDROID.md` do bloco). `AppHeader.kt` também migrado nesta leva — path data manual (`buildLucideIcon`) descontinuado, ícones agora vêm de `R.drawable.lucide_ic_*` |
...
| Header "CÂMARA DE FOCO" (banner + tooltip + QuestFab) | **Fechado (06/09)** — Bloco A+B+C commit `a3d848e`. Banner com gradiente amber→purple 5% opacidade, ícone Timer + "CÂMARA DE FOCO" em Cinzel/Champagne400, botões de tooltip "?" e QuestFab (scroll) com AbsolutePositioning no Box do header. Build real: `BUILD SUCCESSFUL`, suíte 458/458 PASSED (6 screenshot tests FAILED — pré-existentes, Roborazzi dependency mismatch, não relacionados) | Fiel — header equivale ao `div` de `App.tsx:2338-2381`, tooltip replicado com `AnimatedVisibility`/`fadeIn`/`fadeOut`, QuestFab abre `HeroLogModal` com `dailyQuests()`/`guildQuestsProcessed()` (barra de progresso, estilos de claim) + botão "Ir para Painel de Contratos →" que navega via `onNavigateToTab`. `FocusOrbPreviewScreen` ganhou parâmetro `onNavigateToTab: (String) -> Unit` | **Não validada** — build OK, teste OK, sem print real em device | React: `App.tsx:2338-2381` (header banner), `QuestFab.tsx` (modal com contratos), `Modal.tsx:53-75` (variant amber). Fontes Cinzel/JetBrains Mono agora usadas (`res/font/cinzel_*.ttf`, `res/font/jetbrains_mono_*.ttf`) — `Type.kt` já definia `Cinzel`/`JetBrainsMono`, mas muitos componentes usavam `FontFamily.Serif`/`.Monospace` (sistema); o header usa `Cinzel` corretamente. Bug corrigido durante implementação: `AnimatedVisibility` resolvia para `ColumnScope.AnimatedVisibility` dentro de `BoxScope` → usado caminho completo `androidx.compose.animation.AnimatedVisibility` |

## 4. Módulo Personagem

| Item | Status | Fidelidade Lógica | Fidelidade Visual | Notas |
|---|---|---|---|---|
| Ficha de personagem | Validado | Validada (usa `CombatLogic.requiredXpForCombatLevel`, `TITLE_CATALOG`) | Parcial — screenshot em modo record, 2 estados cobertos, ainda sem inspeção visual humana | React: `CharacterScreen.tsx` (só versão mobile portada). Bloco 10. `TitleEquipModal` real ainda pendente (placeholder no lugar) |
...
| Rollover de dia | **Completo (Bloco E fechado, 20/08)** | Fiel + 2 decisões conscientes de desenho (não fidelidade estrita): consumo único de `StreakShield` por rollover (corrige bug real da fonte, que permitia proteção dupla de graça) e proteção tripla estendida (streak global + dano de HP + `Daily.streak` individual, decisão de Bruno — na fonte `Daily.streak` sempre zerava incondicionalmente). Delay de 100ms antes de `isPlayerDead=true` mantido (equivalente ao `setTimeout` da fonte) | N/A (sem UI ainda) | `RolloverLogic.kt`, wired no `init()` do `HeroLogViewModel.kt` — é o único dos 4 arquivos do módulo Missões (Habit/Daily/Todo/Rollover) que estava corretamente wireado e usando `model.*` desde a criação. 12/12 (`RolloverLogicTest`) + 36/36 (`HeroLogViewModelTest`). Pendências registradas: (a) tornar `StreakShield` mais raro/difícil de obter; (b) migrar armazenamento do shield de `inventory: List<Item>` genérico para contador dedicado (`streakShieldCount`, estilo Duolingo) — ambas antes do port ser considerado "terminado" |
...
| Contratos (UI — `QuestsScreen.kt`) | **Validado (24/08)** — wiring (Bloco I) + correção de fidelidade visual (24/08, segunda rodada) auditados linha por linha contra `QuestsTab.tsx` + wrapper `App.tsx`. Build/testes reportados verdes pelo AI Studio (não verificável de forma independente por Claude, sem Android SDK no sandbox) | Fiel — título "CONTRATOS"/ícone `Layers`, sub-abas "Contratos Diários"/"Marcos da Jornada", legendas contextuais exatas, paleta 100% âmbar em progresso (esmeralda só no XP e no badge "Concluído", igual à fonte), textos de rodapé literais ("Reivindicar"/"Baú de espólios recolhido"/"Desbloqueia ao atingir progresso"), opacidade 0.6 do card inteiro quando resgatado, bolinha pulsante via `rememberInfiniteTransition` | N/A | **Primeira entrega (Bloco I) tinha divergência real de fidelidade** — textos, ícone, cores e emojis parcialmente inventados em vez de copiados da fonte; corrigido numa segunda rodada com prompt literal citando o texto exato do React. Registrar como padrão: pedir fidelidade "linha por linha" no prompt não garante que o AI Studio siga — auditoria pós-entrega continua obrigatória mesmo com a instrução explícita |
...
| UI (telas de Habits/Dailies/Todos) | **H1 (Habits), H2 (Dailies) e H3 (Todos) fechados, lógica/estrutura — H3 commitado no GitHub (`vosslerazelas22/HeroLog-Android-2`, commit `fe6f29d`), 2 dos 3 achados resolvidos (22/08)** | N/A (componente apresentacional, chama callbacks do ViewModel já testados) | **NÃO validado — Google AI Studio não conseguiu entregar os PNGs do Roborazzi (H1, H2 e H3).** Total agregado confirmado (343/343 no H3, 331 do H2 + 12 novos). Mesmo assim, sem print real. **Fica marcado "Fidelidade Visual: Não validada" até print real comparado com o React (dois viewports de referência) chegar** | `HabitsScreen.kt` + `DailiesScreen.kt` + `TodosScreen.kt` + `ScoreColor.kt` (compartilhado) + `DifficultyBadge.kt` (extraído de dentro de `HabitsScreen.kt` no início do H2, compartilhado pelos três). H1: 3 desvios corrigidos (tags no card, streak 🔥 no card, `allowBackdropClose=false`). H2: 4 desvios corrigidos no checklist expandido (cor do texto completo invertida, ícone Check indevido, tamanho/cor de checkbox, borda do chevron em 4 lados). **H3: 3 achados de auditoria contra `TodosTab.tsx` real — decisão de Bruno em 22/08**: (1) **aceito como melhoria consciente, não corrigido**: guard de filtro desabilitado usa `isModalOpen` (criação OU edição); fonte usa só `editingTodo !== null` — filtro fica habilitado durante criação de item novo no React, motivo provável é evitar que o item em edição suma da lista ao trocar filtro (risco que não existe na criação, por não haver item "atual" na lista). Erro de especificação no prompt, não do AI Studio — invisível na prática porque o backdrop do modal cobre os filtros nos dois modos, mas diverge da fonte literal; decisão de Bruno: manter como está; (2) **melhoria consciente**: texto do item de checklist expandido é clicável no Android (chama toggle), fonte só tem clique no checkbox — área de toque maior, decisão deliberada, não é desvio a corrigir; (3) **corrigido (22/08)**: cor de fundo do card concluído usava `emerald-700` em baixa opacidade (`0x0D047857`); trocado para `Color(0x08022C22)` (`emerald-950` a 3% de opacidade, `0.03 × 255 ≈ 8` = `0x08`), fiel 1:1 à fonte (`emerald-950/[0.03]`) — border (`emerald-500/30%`) já batia exato e não foi tocado — ver `DEV_LOG_ANDROID.md` Blocos H1/H2/H3. Código final do H2 e H3 reconferido direto no repositório real (`git clone` + `bash_tool`) após commits de Bruno |
| Wiring das telas H1/H2/H3 na navegação real (`MainActivity`) | **Em andamento (22/08)** — bloqueado por falta de navegação estruturada até este bloco; resolvido junto com "Navegação inferior (mobile)" (seção 2). `HabitsScreen`/`DailiesScreen`/`TodosScreen` agora chamadas de dentro do `Scaffold` com `state.habits`/`state.dailies`/`state.todos` reais e todos os callbacks do `HeroLogViewModel` (`triggerHabit`, `toggleDaily`/`toggleTodo`, `add*`/`edit*`/`delete*`, `toggle*ChecklistItem`) | N/A (wiring puro, sem lógica nova — os callbacks já eram testados no Bloco G/F) | **NÃO validado** — sem build real, sem inspeção visual no dispositivo | Antes deste bloco, as 3 telas existiam prontas no código mas nunca apareciam no app rodando — confirmado por auditoria direta do `MainActivity.kt` real (nenhuma referência a `HabitsScreen`/`DailiesScreen`/`TodosScreen` antes de 22/08). Sub-abas de Missões ainda não portadas (`quests`/`history`) e todo o módulo Reino (`shop`/`titles`/`heatmap`/`stats`/`achievements`/`logs`/`guide`) apontam pra `PlaceholderScreen` — decisão explícita de Bruno em 22/08 pra não bloquear o wiring de Habits/Dailies/Todos esperando módulos ainda não implementados |
...
| Histórico / Crônicas Diárias (UI — `HistoryScreen.kt`) | **Validado (24/08, Bloco J)** — sem lógica nova (dados já vinham de `FocusApplyLogic.kt`), só UI. Duas rodadas: primeira reprovada por reaproveitar o mesmo card pros dois modos de visualização (a fonte tem dois layouts distintos); segunda corrigida com prompt literal, auditada linha por linha contra `HistoryTab.tsx` + wrapper `App.tsx` | Fiel — estrutura de dois cards reais (`NotesHistoryCard` compacto vs `FullHistoryCard` completo), textos literais ("Crônicas completas"/"Compilado de Notas", badge "TERRA SELVAGEM", "Revelar/Ocultar Crônica Mística", "Nenhuma crônica antiga disponível"), emojis corretos (⚡ XP esmeralda, 💎 GP âmbar) | N/A | 3 divergências cosméticas aceitas conscientemente (não bloqueiam): (1) alguns textos secundários usam `Stone400` onde a fonte usa `amber-100` em alpha baixo; (2) painel expansível da crônica ganhou um título "📜 Crônica Mística" que não existe na fonte; (3) gradiente do painel expansível ficou horizontal, na fonte é vertical (`bg-gradient-to-b`). React: `HistoryTab.tsx` |
