...
| `HeroLogModal` (componente de modal reutilizável) | **Em correção (spec-002)** — passo 1 fechado 16/09: PoC Haze + decisão Opção A em device real (blur cobre header, painel nítido, tint ok, 0 jank; raio final 0.25 dp noise 0; Voltar corrigido sistemicamente; API 24 fallback chapado). FR-001/FR-002 (passo 2) pendentes | Fiel — props, `ModalCountRegistry`, `BackHandler`-dentro-do-`Dialog` com guard `isOpen` (BH-1 fechado). Device: Voltar fecha (desbloqueado) / não fecha (bloqueado), incl. produção | **Backdrop validado em device; restante (sizing §4) pendente** — baseline Roborazzi registra o bug do painel esticado, não é prova | Bloco 3 + 3.1. Port de `Modal.tsx`. `incrementModalCount`/`decrementModalCount` resolvidos (Bloco 6.2, `ModalCountRegistry`). D1–D11 mapeados na spec-002 (sizing, backdrop, tipografia, paddings, fechar, glow, sombra, animação). Backlog mesma causa raiz: `LevelUpOverlay.kt` (sprint própria). Ver `DEV_LOG_ANDROID.md` 16/09 |
