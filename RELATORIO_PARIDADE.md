...
| D200 | **RaidModeInfoBox — cores do texto inline** | Cores literais do React (`App.tsx:2791–2862` + `index.css:14–18`): principal Padrão `champagne-400` `#E5C158`, Masmorra `purple-300` `#D8B4FE`, Selvagem `red-400` `#F87171` sólido, bônus `zinc-400` `#A1A1AA`, cooldown `purple-400` `#C084FC` sólido, `?` com borda `*-500/20` + texto sólido | Tokens `Info*` dedicados com valores literais do React; `InfoRow` com `helpTextColor` separado; fundo do `?` transparente (precedente: tooltip pomodoro `MainActivity.kt:1672–1688`). Tokens dos botões do segmented control intocados | **Resolvido (PR #20, 19/09)** | `RaidModeSection.kt` (+33/−11). 5 baselines Roborazzi regravados. 609/609 testes. Visual device: PENDENTE |
...
| # | Item | React | Android | Severidade | Notas |
|---|---|---|---|---|---|
| D155 | **Backdrop blur** | `backdrop-blur-md` (~12px) + `bg-black/90` | `Color.Black.copy(alpha=0.9f)` sem blur | **DESVIO CONSCIENTE (fase 2, 21/09)** | Cor alinhada no bloco 19/09; blur aceito como desvio — sob preto a 90% o blur é praticamente invisível |
...
| D159 | **Combat "Você evoluiu!" size** | `text-[10px] sm:text-xs` (10-12sp responsivo) | `fontSize = 10.sp` (igual ao mobile `text-[10px]`) | ~~**Baixa**~~ **RESOLVIDO (bloco levelup-audit 20/09, device pendente)** | Tamanho mobile alinhado (era 11.sp) |
...
| D162 | **Skill "alcançou o Nível X" highlight** | `text-emerald-400 text-lg font-black` (destaque verde) | `Text` separado `"Nível {n}"` 18sp Black emerald em linha própria (span `block` do mobile) | ~~**Média**~~ **RESOLVIDO (bloco levelup-audit 20/09, device pendente)** | Mesmo padrão Dx-2 do combat |
| D201 | **Card esticado até a altura da tela** | Card com altura do conteúdo (sem `h-full`) | Aura com `fillMaxSize()` dentro de `BoxWithConstraints` herdava o max da tela e esticava o card | ~~**Alta**~~ **RESOLVIDO (c8484e6)** | Causa raiz: filho `fillMaxSize` em `BoxWithConstraints` mede o max, não o conteúdo. Fix = container `Box` + aura/partículas com `matchParentSize()` + `BoxWithConstraints` só para medir as partículas. A auditoria D1–D13 comparava valores literais e não pegou o layout esticado — lição registrada |
| D202 | **R-1: margem `p-4` do backdrop** | Container `fixed` com `p-4` (`App.tsx:4468`) | Dialog sem padding (card edge-to-edge) | ~~**Baixa**~~ **RESOLVIDO (c8484e6)** | `Modifier.padding(16.dp)` no `AnimatedVisibility` |
| D203 | **D11: Voltar engolido** | N/A (fonte não trata teclado; só o botão fecha) | `BackHandler` fora do `Dialog` registrado no dispatcher da activity era engolido (`dismissOnBackPress=false`) — Voltar nunca fechava | ~~**Alta**~~ **RESOLVIDO (957e7a2)** | `BackHandler` movido para DENTRO do `Dialog` (janela focada; mesmo padrão HeroLogModal/DailyReportModal, spec-002). Divergência consciente vs React mantida: Voltar agora fecha (antes não fechava nada) |
| D204 | **D5: line-height** | Combat `<p>`: `leading-relaxed` (1.625) em `text-sm` (14px) → 22.75 (`App.tsx:4524`); skill nome: `leading-tight` (1.25) em `text-base` (16px) → 20 (`App.tsx:4596`) | Sem `lineHeight` | ~~**Baixa**~~ **RESOLVIDO (fase 2, 21/09, device pendente)** | `lineHeight = 22.75.sp` / `20.sp` inline; sem `lineHeightStyle` (inexistente no projeto; default equivale ao CSS). Residual: spans `NÍVEL` herdam leading unitless na fonte (line box 29.25px) — fora de escopo |
| D205 | **D7: drop-shadow dos títulos** | `drop-shadow-[0_2px_10px_rgba(226,176,84,0.4)]` (`App.tsx:4521`) e `rgba(52,211,153,0.35)` (`App.tsx:4593`) | Ausente | ~~**Média**~~ **RESOLVIDO (fase 2, 21/09, device pendente)** | `TextStyle(shadow = Shadow(...))` no próprio Text (convenção PARIDADE L67/issue #20); conversão 1:1 igual a `FocusCompletionFlow.kt:219` (`0_2px_12px` → `Offset(0,2)` + `blurRadius 12f`); nunca `Modifier.shadow` |
| D206 | **Padding/espaçamentos do conteúdo** | Wrapper `p-6` (24) + `space-y-6` (24) + `space-y-2` (8) + span `block mt-1` (4) + divider `my-3` (12) + botão `pt-2` (8) | Padding 24dp; spacers 24/8/4; divider assimétrico top 12 / bottom 24 (colapso `max(12,24)` + `pt-2` 8 = 32 até o botão); `widthIn` 384dp (`max-w-sm`); MAESTRIA→skillName `Spacer(8.dp)` | ~~**Média**~~ **RESOLVIDO (fase 2, 21/09 + bloco AMBÍGUO, device pendente)** | Topo do divider = 12 (Tailwind do React é v4, lock 4.3.1). MAESTRIA→skillName resolvido: evidência empírica (CSS compilado TW 4.1.14/4.3.1, lock 4.3.1) — `space-y` = `margin-block-end` em `:not(:last-child)`, colapso max(8,4) = 8 |
| D207 | **D8: pico do glow (skill)** | `emeraldRadiance` pica em 60px/0.55 (`index.css:81`) | Glow alpha 0.3↔0.65 nas duas variantes | **DESVIO CONSCIENTE (fase 2, 21/09)** | Aproximação de glow deliberada (Dx-16): só alpha pulsa, blur fixo |
| D208 | **D9: sombra do botão** | `shadow-lg` no botão (`App.tsx:4533`) | Sem shadow no `ContinueButton` | **DESVIO CONSCIENTE (fase 2, 21/09)** | Sombra preta a 10% sobre fundo escuro não aparece |
| D209 | **D13: fade das sparks** | `riseFast`: opaco até 85%, some em 100% (`index.css:55-57`) | Fade a partir de 70% (`LevelUpOverlay.kt`) | **DESVIO CONSCIENTE (fase 2, 21/09)** | Diferença mínima |
| D210 | **D12: gradiente linear de topo do card** | Inexistente (card só tem `bg-stone-950`; brilho vem do painel radial) | `.background(Brush.verticalGradient(glow→Transparent))` (Dx-5) | **ADIADO (fase 2, 21/09)** | Adição sem contraparte na fonte; decisão pendente em momento posterior |
| D211 | **line-height embutido nos `text-*` do Tailwind não portado** | `text-2xl` = 32px de line box (LEVEL UP!), `text-lg`/`text-xl` = 28px, `text-base` = 24px, `text-sm` = 20px, `text-[10px]` herda 1.5 do preflight = 15px | Só os `leading-*` foram portados (D204); demais textos usam o default do Compose | **ABERTO (device pendente)** | Decidir após comparação lado a lado no device |
...
| ✅ Resolvido por bloco levelup-audit-20/09 (sem PR; device pendente) | 6 | D162 FECHADO (span "Nível X" skill virou `Text` separado 18sp Black emerald — mesmo padrão Dx-2 do combat) + D1/D2/D4/D10 da auditoria 20/09 (eyebrow 11→10sp, "LEVEL UP!" 26→24sp, botão 12→10sp, divider 90dp fixos → `fillMaxWidth(1/3)` = `w-1/3`); 2 baselines regerados/inspecionados via PNG; FULL 615/615 |
...
| ✅ Resolvido por c8484e6 + 957e7a2 (21/09, sem PR; device pendente) | 3 | D201 (card esticado — aura `matchParentSize`), D202 (R-1 margem `p-4` 16dp), D203 (D11 `BackHandler` dentro do `Dialog`) |
| ✅ Resolvido por bloco levelup-fase-2 21/09 (sem PR; device pendente) | 3 | D204 (D5 line-height 22.75/20sp), D205 (D7 drop-shadow Shadow 1:1), D206 (padding 24dp + spacers 24/8/4 + divider top 12/bottom 24); D155/D207/D208/D209 aceitos como DESVIO CONSCIENTE, D210 ADIADO, Cinzel N/A (index.html carrega 400–800, navegador também renderiza 900 como 800) |
| ♻️ Refactor ae70198 (21/09, sem PR) | — | `DailyReportModal` migrado para `Dialog` + `BackHandler` dentro (paridade system bars, padrão HeroLogModal/LevelUpOverlay) + `BoxWithConstraints`/`WindowInsets.systemBars` com altura real; 7 baselines `daily_report_*` regravados — ver entrada curta no DEV_LOG |
