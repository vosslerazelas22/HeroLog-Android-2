...
| D206 | **Padding/espaçamentos do conteúdo** | Wrapper `p-6` (24) + `space-y-6` (24) + `space-y-2` (8) + span `block mt-1` (4) + divider `my-3` (12) + botão `pt-2` (8) | Padding 28/32dp; `Spacer(4.dp)` ícone→texto; zero entre textos; divider simétrico 12; divider→botão 20 | ~~**Média**~~ **RESOLVIDO (fase 2, 21/09, device pendente)** | Padding 24dp; spacers 24/8/4; divider assimétrico top 12 / bottom 24 (colapso `max(12,24)` + `pt-2` 8 = 32 até o botão). AMBÍGUOS v3/v4 intocados: topo do divider (8 em v3, 12 em v4) e MAESTRIA→skillName (`mt-1` 4 em v4, `space-y-2` 8 em v3) |
